...
isBooked: false,
      ...
      socket.on('requestToDoctor', (onConnect)=>{
        setSlotData(slotData.map(obj => {
          if (obj.slot.toString() === onConnect.data.timeStamp) {
            return { ...obj, status : 'pending' };
          }
          return obj; 
        }));
        setId(onConnect.data._id)
        setTimeStamp(onConnect.data.timeStamp)
  
      });

      socket.on('au', (onConnect)=>{
        setSlotData(slotData.map(obj => {
          if (obj.slot === onConnect.obj) {
            return { ...obj, status : 'pending' };
          }
          return obj; 
        }));
  ...
  const [pickeDate, setPickedDate] = useState(dayjs('2023-06-17'));
...
const handleEditSlot = (slotIndex,input)=>{

  if(input === false){
    const newStartTime = prompt('Enter new start time (e.g., 1:00):');
    const newStartMeridian = prompt('Enter new start meridian (e.g., am, pm):');
    const newEndTime = prompt('Enter new end time (e.g., 1:30):');
    const newEndMeridian = prompt('Enter new end meridian (am, pm):');
    
    // Update the timing of the clicked slot
    setSlotData((prevSlotData) =>
      prevSlotData.map((slot, index) => {
        if (index === slotIndex) {
          return {
            ...slot,
            timeStart: newStartTime,
            startMeridian : newStartMeridian,
            timeEnd: newEndTime,
            endMeridian : newEndMeridian
          };
        }
        return slot;
      })
    ...
    // Find the affected slots and mark them as booked
    setSlotData((prevSlotData) =>
      prevSlotData.map((slot, index) => {
        if (
          index !== slotIndex &&
          slot.timeStart < newEndTime &&
          slot.startMeridian === newStartMeridian &&
          slot.timeEnd > newStartTime &&
          slot.endMeridian === newEndMeridian
        ) {
          return {
            ...slot,
            isBooked: true,
    ...
    alert("make slot unavailable first")
  }
...
setSlotData(slotData.map(obj => {
  ...
  setSlotData(slotData.map(obj => {
    if (obj.slot.toString() === timeStamp) {
      return { ...obj, status : 'reshedule', timeStart : updatedTime.timeStart, timeEnd : updatedTime.timeEnd };
    }
    return obj; 
  }));
...
// console.log(slotData);
  ...
  <div className="flex items-center justify-center space-x-2">
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
        ...
        <TimePicker label="From" onChange={handleTimeSlot1}/>
    ...
    <LocalizationProvider dateAdapter={AdapterDayjs}>
        ...
        <TimePicker label="To" onChange={handleTimeSlot2}/>
        ...
        <div className="grid md:grid-cols-3 sm:grid-cols-2 grid-cols-1">
          <div className="flex flex-col justify-around">
            ...
            <div>
              <div className="w-2/3">
                <LocalizationProvider dateAdapter={AdapterDayjs}>
                  <DatePicker value={pickeDate} onChange={(newValue)=>handleCalender(newValue)}/>
                </LocalizationProvider>
              </div>
            </div>
          </div>
...
{/* ---------------------------slot start---------------------- */}
          ...
          <div className="grid grid-cols-4 gap-4 mt-7 w-11/12 px-4 py-1 h-44  overflow-y-scroll mx-auto">
            {slotData.map((elm, index) => (
              ...
              <div className='relative  py-[10px] px-2 list-none font-medium tracking-wider text-base rounded-[40px] neoSlot hover:scale-100 flex justify-center space-x-2 items-center'  onMouseEnter={()=>handelEnter(index)} onMouseLeave={()=>handleLeave(index)}  key={index}>
...
<span  className={`${elm.availability === false ? "text-[#bfc2c5]" : "text-[#263442]"}`} onClick={()=>{handleEditSlot(elm.slot,elm.availability)}} >
    ...
    <div
      data-tooltip={`${
        elm.status === "notBooked"
          ...
          : elm.status === "booked"
          ? "Congratulatins, appointment is confirmed"
          : elm.status === "resheduled"
          ? "appointment is recheduled"
          : elm.status === "cancel"
          ? "appointment is cancelled by you"
          : ""
      }`}
      className={`${
        elm.status === "notBooked"
          ? ""
          : elm.status === "pending"
          ? "tooltip h-4 w-4 bg-red-600 animate-ping"
          : elm.status === "booked"
          ? "tooltip h-4 w-4 bg-green-600"
          : elm.status === "reshedule"
          ? "tooltip h-4 w-4 bg-yellow-500"
          : elm.status === "cancel"
