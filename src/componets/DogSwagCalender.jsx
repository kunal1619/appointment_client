  ...
  const handleRemoveItem = (item) => {
    ...
    socket.on('isAccepted', (onConnect)=>{
      setSlotData(slotData.map(obj => {
        if (obj.slot.toString() === onConnect.timeStamp){
    ...
    socket.on('reScheduledTouser', (onConnect)=>{
      setSlotData(slotData.map(obj => {
        if (obj.slot.toString() === onConnect.timeStamp){
          return { ...obj, status : 'reshedule', timeStart : onConnect.updatedTimeSlot.timeStart, timeEnd : onConnect.updatedTimeSlot.timeEnd };
        }
        return obj; 
      }));
...
// console.log(formData);
  ...
  const showModalFunction = (a, b, c, d, input)=>{
    ...
    if(types !== "category" && selectedItems.length !== 0){
      setShowModal(true);
      setOpen1(true);
    setFormData({
  docId : 1,
  ...
  setSlotNumber(input)
      ...
      alert("select category, services and date first")
    }
 ...
 const [showAcptRejct, setShowAccptRej] = useState(false);
 const [slotNum, setSlotNum] = useState(null)

 const handleAcptRjct = (index)=>{
  setSlotNum(index)
  ...
  return (
    <div className='w-full min-h-screen flex justify-center items-center bg-[#f2f3f7]'>
        ...
        <div className="absolute h-screen w-full bg-gray-600 flex justify-center items-center z-10 bg-opacity-40">
          <div className="h-[40%] w-1/3 bg-white rounded-md p-10">
...
<LocalizationProvider dateAdapter={AdapterDayjs}>
      <DatePicker value={date} onChange={(newValue)=>handleCalender(newValue)}/>
...
{ slotData.length > 0 ? slotData.map((elm, index)=>{
